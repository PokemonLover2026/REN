...
class RVN_specifications:
    def __init__(self):
        self.Box = None
        self.rho = None
        self.robustness_type = arguments.Config['specification']['robustness_type']
        self.norm = arguments.Config['specification']['norm']
        self.epsilon = arguments.Config['specification']['epsilon']
        self.epsilon_min = arguments.Config['specification']['epsilon_min']

    def load_from_vnnlib(self, vnnlib):
        Box = vnnlib[-1][0]
        rho = vnnlib[-1][-1][-1][-1][-1]
        self.update_Box(Box)
    ...
    def update_Box(self, Box):
        ...
        if self.Box is None:
            ...
            for last_bound, current_bound in zip(self.Box, Box):
                low_b = min(current_bound[0], last_bound[0])
                up_b = max(current_bound[1], last_bound[1])
                updated_Box.append((low_b, up_b))
            self.Box = updated_Box

    def update_norm_epsilon(self, norm, epsilon):
        if norm is None:
            pass
        else:
