...
def attack(model_ori, x, vnnlib, verified_status, verified_success,
    ...
    if 'auto_attack' not in arguments.Config["attack"]["attack_mode"]:
        ...
        attack_ret, attack_images, attack_margins, all_adv_candidates, pgd_input = attack_function(
    ...
    print(f'The pure sample is {x} and the PGD delta is {attack_delta}')

    # attack_images has shape (batch, spec, c, h, w).
    return verified_status, verified_success, attack_images, attack_margins, all_adv_candidates, pgd_input
...
def test_conditions(input, output, C_mat, rhs_mat, cond_mat, same_number_const, data_max, data_min, return_success_idx=False):
    ...
    if same_number_const:
        ...
        a = torch.matmul(C_mat, output.unsqueeze(-1)).squeeze(-1)
...
def default_adv_example_finalizer(model_ori, x, best_deltas, data_max, data_min, C_mat, rhs_mat, cond_mat):
    ...
    return attack_image, attack_output, attack_margin, pgd_input
...
def pgd_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat,
    ...
    attack_iters = arguments.Config["change"]["attack_iters"]
    num_restarts = arguments.Config["change"]["num_restarts"]
    ...
    delta_lower_limit = data_min
    delta_upper_limit = data_max
    ...
    for _ in range(attack_iters):
        inputs = torch.clamp(normalize(X  +  delta), data_min, data_max)
...
def attack_with_general_specs(model, x, data_min, data_max,
    ...
    batch_size = arguments.Config["change"]["batch_size"]
    num_restarts = arguments.Config["change"]["num_restarts"]
    ...
    for _ in tqdm(range((num_restarts + batch_size - 1) // batch_size)):
        if arguments.Config["rvn_setting"]["attack_method"] == "CW":
            best_deltas_, last_deltas, best_loss_, early_stopped = CW_attack_with_general_specs(
                model, x, data_min, data_max, C_mat, rhs_mat, cond_mat)
        elif arguments.Config["rvn_setting"]["attack_method"] == "PGD":
            ...
            raise ValueError("unknown attack detected!")
    ...
    attack_image, attack_output, attack_margin, pgd_input= eval(arguments.Config["attack"]["adv_example_finalizer"])(
    ...
    if test_conditions(attack_image.unsqueeze(1), attack_output.unsqueeze(1),
        ...
        return True, attack_image.detach(), attack_margin.detach(), all_adv_candidates, pgd_input
    else:
        print("PGD attack failed")
        return False, attack_image.detach(), attack_margin.detach(), all_adv_candidates, pgd_input
...
def CW_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat, cond_mat):
    """
    :param model:
    :param x:the input tensor.
    :param c:the confidence parameter.
    :return: best_deltas_, last_deltas, best_loss_, early_stopped
    """
    batch_size = arguments.Config["attack"]["pgd_batch_size"]
    num_restarts = arguments.Config["attack"]["pgd_restarts"]
    early_stop=arguments.Config["attack"]["pgd_early_stop"]
    batch_size = min(batch_size, num_restarts)
    same_number_const = True
    early_stopped = False
    ...
    num_classes = C_mat.shape[-1]
    num_or_spec = len(cond_mat[0])
    extra_dim = (num_restarts, num_or_spec)
    ...
    for iteration in range(max_iter):
        ...
        loss, loss_gama = CW_loss(X, inputs, output, C_mat, rhs_mat, lamda, batch_size, confidence, modifier)
        ...
        with torch.no_grad():
            ...
            modifier_grad_sign = modifier.grad.clone().sign()
            modifier += (1 - lr_decay) * modifier_grad_sign
            modifier.clamp_(data_min, data_max)
            modifier.grad.detach_()
            modifier.grad.zero_()
        ...
        if eval(arguments.Config["attack"]["early_stop_condition"])(inputs, output, C_mat, rhs_mat,
                                                                    cond_mat, same_number_const, data_max, data_min,
                                                                    model, indices, num_or_spec).all():
            print("pgd early stop")
            early_stopped = True
            break
...
def CW_loss(X, inputs, output, C_mat, rhs_mat, lamda, batch_size, confidence, modifier):
    C_mat = C_mat.view(C_mat.shape[0], 1, output.shape[2], -1, C_mat.shape[-1])
    # [num_example, 1, num_or_spec, num_and_spec, num_output]
    rhs_mat = rhs_mat.view(rhs_mat.shape[0], 1, output.shape[2], -1)
    loss = C_mat.matmul(output.unsqueeze(-1)).squeeze(-1) - rhs_mat + (lamda / 2) * torch.norm(inputs - X, p = arguments.Config["specification"]["norm"])
    loss = torch.clamp(loss, min=0.01)
    # [num_example, num_restarts, num_or_spec, num_and_spec]
    loss1 = -loss
    loss2 = torch.sum(torch.norm(modifier.view(batch_size, -1), arguments.Config["specification"]["norm"]))
    loss_gama = torch.sum(loss1) + confidence * loss2
    return loss1, loss_gama
