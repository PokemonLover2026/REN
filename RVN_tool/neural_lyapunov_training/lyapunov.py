...
class LyapunovDerivativeSimpleLoss(nn.Module):
    ...
    def forward(self, x, save_new_x=False):
        ...
        for i in range(arguments.Config['rvn_setting']['steps']-1):
            u = self.controller(new_x)
            new_x = self.dynamics.forward(new_x, u)
        if save_new_x:
            self.new_x = new_x
...
class LyapunovDerivativeDOFSimpleLoss(nn.Module):
    ...
    def forward(self, xe):
        ...
        for i in range(arguments.Config['rvn_setting']['steps']-1):
            x = self.new_xe[:, :self.nx]
            e = self.new_xe[:, self.nx:]
            z = x - e
            y = self.observer.h(x)
            ey = y - self.observer.h(z)
            u = self.controller.forward(torch.cat((z, ey), dim=1))
